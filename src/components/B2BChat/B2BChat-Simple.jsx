...
const B2BChat = ({ userId, isAuthenticated = false }) => {
  ...
  useEffect(() => {
    if (isOpen && messages.length === 0) {
      addBotMessage('¡Hola! 👋 Soy tu asistente B2B de ImBlasco. ¿En qué puedo ayudarte hoy?')
    }
  }, [isOpen])
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      addBotMessage(`⚠️ Lo siento, hubo un error al procesar tu mensaje. Por favor, intenta de nuevo.`)
    } finally {
      setIsLoading(false)
